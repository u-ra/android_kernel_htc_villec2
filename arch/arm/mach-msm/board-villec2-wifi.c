...
#include "board-villec2-wifi.h"

int villec2_wifi_power(int on);
int villec2_wifi_reset(int on);
int villec2_wifi_set_carddetect(int on);
int villec2_wifi_get_mac_addr(unsigned char *buf);
...
static void *villec2_wifi_mem_prealloc(int section, unsigned long size)
{
...
int __init villec2_init_wifi_mem(void)
{
	int i;
...
static struct wifi_platform_data villec2_wifi_control = {
	.set_power      = villec2_wifi_power,
	.set_reset      = villec2_wifi_reset,
	.set_carddetect = villec2_wifi_set_carddetect,
	.mem_prealloc   = villec2_wifi_mem_prealloc,
	.get_mac_addr	= villec2_wifi_get_mac_addr,
};
...
static struct platform_device villec2_wifi_device = {
	...
	.dev            = {
		.platform_data = &villec2_wifi_control,
	},
};

static unsigned villec2_wifi_update_nvs(char *str)
{
...
int villec2_wifi_get_mac_addr(unsigned char *buf)
{
	...
	printk(KERN_INFO"villec2_wifi_get_mac_addr = %02x %02x %02x %02x %02x %02x \n",
...
#else
	villec2_wifi_update_nvs("sd_oobonly=1\n");
#endif
	villec2_wifi_update_nvs("btc_params80=0\n");
	villec2_wifi_update_nvs("btc_params6=30\n");
	villec2_init_wifi_mem();
	ret = platform_device_register(&villec2_wifi_device);
	return ret;
}
