...
#include "sd_ops.h"
#include "sdio_ops.h"
#define MMC_BKOPS_MAX_TIMEOUT    (2 * 60 * 1000) /* max time to wait in ms */
...
void mmc_start_bkops(struct mmc_card *card)
{
	...
	if (mmc_card_doing_bkops(card) || !mmc_card_need_bkops(card)) {
		if (!mmc_card_need_bkops(card)) {
			spin_lock_irqsave(&card->host->lock, flags);
			mmc_card_clr_bkops_during_suspend(card);
			spin_unlock_irqrestore(&card->host->lock, flags);
		}
		return;
	}
...
int mmc_can_discard(struct mmc_card *card)
{
	...
	if (card->cid.manfid == 0x15) {
		if (card->ext_csd.sectors == 30535680 ||
			...
			/* For Samsung VFX-U eMMC, discard command is not supported.
			   So, we check PRV for this. */
...
int mmc_bkops_resume_task(struct mmc_host *host)
{
	...
	if (host->bkops_timer.need_bkops) {
		...
		if (time_bkops_end > time_bkops_start) {
			...
			printk(KERN_DEBUG "need_bkops = %u, time_diff = %lu\n",
			host->bkops_timer.need_bkops, (long unsigned int)time_diff);
		}
	}
	spin_unlock_irqrestore(&host->lock, flags);
	...
	if (!err) {
		printk(KERN_DEBUG "%s: card status %x needbkops %u\n",
			__func__, status, host->bkops_timer.need_bkops);
		if (!(status & R1_READY_FOR_DATA) ||
			(R1_CURRENT_STATE(status) == R1_STATE_PRG)) {
			err = host->bus_ops->housekeeping(host, 0);
			if (err)
				printk(KERN_ERR "%s: fail to send HPI!!\n", __func__);
			else {
				do {
					err = mmc_send_status(host->card, &status);
					if (err) {
					...
					if (query_count-- == 0) {
						...
						(R1_CURRENT_STATE(status) == R1_STATE_PRG));
				if (card_not_ready) {
					printk(KERN_ERR "%s: card not ready status %x err %d needbkops %u\n",
						__func__, status, err, host->bkops_timer.need_bkops);
					err = -EBUSY;
				} else {
					if (host->bkops_timer.need_bkops == 0)
						host->bkops_count++;
					printk(KERN_DEBUG "%s: card ready status %x needbkops %u bkops_count %d\n",
						__func__, status, host->bkops_timer.need_bkops, host->bkops_count);
				}
			}
		} else {
			host->bkops_count++;
			printk(KERN_DEBUG "%s: needbkops %u, bkops_count %d\n",
						__func__, host->bkops_timer.need_bkops, host->bkops_count);
			host->bkops_timer.need_bkops = 0;
		}
	} else
		printk(KERN_ERR "%s: mmc_send_status fail err= %d\n", __func__, err);
	mmc_release_host(host);
	return err;
}
